//go:build ZZZZZZZZZZZZZZZZZZZZZZZZZZ

// ^ Disable building this file temporarily
...
type DebugPlaybackDevice struct {
	...
	ActiveTrack          DebugTrack
	startTrackSwitcher   sync.Once
}

//const DefaultGain float32 = 1.0

func (pd *DebugPlaybackDevice) getStatus() DeviceStatus {
	pos := 0
...
// NewDebugPlaybackDevice creates a new playback device which implements all the basic Jukebox mode commands defined here:
// http://www.subsonic.org/pages/api.jsp#jukeboxControl
// Starts the trackSwitcher goroutine for the device.
func NewDebugPlaybackDevice(ctx context.Context, playbackServer PlaybackServer, name string, deviceName string) *DebugPlaybackDevice {
	return &DebugPlaybackDevice{
		serviceCtx:           ctx,
		ParentPlaybackServer: playbackServer,
		User:                 "",
		Name:                 name,
...
func (pd *DebugPlaybackDevice) String() string {
	return fmt.Sprintf("Name: %s, Gain: %.4f, Loaded track: %s", pd.Name, pd.Gain, pd.ActiveTrack)
}

func (pd *DebugPlaybackDevice) Get(ctx context.Context) (model.MediaFiles, DeviceStatus, error) {
	log.Debug(ctx, "Processing Get action", "device", pd)
	return pd.PlaybackQueue.Get(), pd.getStatus(), nil
}

func (pd *DebugPlaybackDevice) Status(ctx context.Context) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Set(ctx context.Context, ids []string) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Start(ctx context.Context) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Stop(ctx context.Context) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Skip(ctx context.Context, index int, offset int) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Add(ctx context.Context, ids []string) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Clear(ctx context.Context) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Remove(ctx context.Context, index int) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) Shuffle(ctx context.Context) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) SetGain(ctx context.Context, gain float32) (DeviceStatus, error) {
...
func (pd *DebugPlaybackDevice) isPlaying() bool {
	return pd.ActiveTrack != nil && pd.ActiveTrack.IsPlaying()
}

func (pd *DebugPlaybackDevice) trackSwitcherGoroutine() {
...
func (pd *DebugPlaybackDevice) switchActiveTrackByIndex(index int) error {
